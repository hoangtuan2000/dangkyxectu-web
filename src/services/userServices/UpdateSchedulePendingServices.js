...
const UpdateSchedulePendingService = {
    getScheduledDateForCar: async (data) => {
        const token = store.getState().currentUser.user.token;
        const accessToken = store.getState().currentUser.user.accessToken;
        return await axiosInstance
            .post(Constants.ApiPath.UpdateSchedulePending.GET_SCHEDULE_DATE_FOR_CAR, data, {
                headers: {
    ...
    getSchedule: async (data) => {
        ...
        const accessToken = store.getState().currentUser.user.accessToken;
        return await axiosInstance
            .post(Constants.ApiPath.UpdateSchedulePending.GET_SCHEDULE, data, {
                headers: {
                    Authorization: `${accessToken} ${token}`,
                },
            })
